...
# ------------------------------------------------------------------------------
# Preliminary set up....
bms = DataFrame(benchmark_name = String[], library = String[], method = String[], time = Float64[], memory = [], alloc = Int64[], neqs = Int64[], notes=String[])
...
eqs  = nash_se2(rand_6x7_StrategicGames,max_samples=Inf)
neqs = length(eqs)
res  = @binfo nash_se2($rand_6x7_StrategicGames,max_samples=Inf)
push!(bms,[game_name,"StrategicGames","nash_se2",res...,neqs,""])
...
solver = pygambit.nash.ExternalEnumPolySolver()
eqs = solver.solve(rand_6x7_gambit)
neqs= length(eqs)
res = @binfo solver.solve(rand_6x7_gambit)
push!(bms,[game_name,"pygambit","ExternalEnumPolySolver",res...,neqs,""])

CSV.write("bms.csv",bms)image.png

# ------------------------------------------------------------------------------
# Test 3 - rand decimal 6x5 game
game_name = "rand_dec_6x5"

# Note that I can't get Gambit working with decima ldata, not even expressed as rational:
...
eqs  = nash_se2(rand_dec_6x5_StrategicGames,max_samples=Inf)
neqs = length(eqs)
res  = @binfo nash_se2($rand_dec_6x5_StrategicGames,max_samples=Inf)
push!(bms,[game_name,"StrategicGames","nash_se2",res...,neqs,""])
...
eqs  = nash_se2(rand_6x7_StrategicGames,max_samples=1)
neqs = length(eqs)
res  = @binfo nash_se2($rand_6x7_StrategicGames,max_samples=1)
push!(bms,[game_name,"StrategicGames","nash_se2",res...,neqs,""])

eqs = nash_se(rand_6x7_StrategicGames,max_samples=1)
neqs = length(eqs)
res = @binfo nash_se($rand_6x7_StrategicGames,max_samples=1)
push!(bms,[game_name,"StrategicGames","nash_se",res...,neqs,""])
